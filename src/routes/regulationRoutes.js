...
router.get('/progress/:userId', async (req, res) => {
    try {
        ...
        const progress = await regulationTracker.analyzeProgress(
            req.params.userId,
            startDate,
            endDate
        );
...
router.get('/patterns/:userId', async (req, res) => {
    try {
        ...
        const patterns = await regulationTracker.identifyPatterns(
            req.params.userId,
            days
        );
        res.json(patterns);
...
router.get('/window-of-tolerance/:userId', async (req, res) => {
    try {
        ...
        const analysis = await regulationTracker.analyzeWindowOfTolerance(
            req.params.userId,
            weeks
        );
        res.json(analysis);
...
router.get('/breakthroughs/:userId', async (req, res) => {
    try {
        ...
        const breakthroughs = await regulationTracker.getRecentBreakthroughs(
            req.params.userId,
            limit
        );
