...
class RegulationTracker {
  ...
  async analyzeProgress(userId, startDate = null, endDate = new Date()) {
    const result = await pool.query('SELECT * FROM analyze_regulation_progress($1, $2, $3)', [
      userId,
      startDate,
      endDate,
    ]);
  ...
  async identifyPatterns(userId, days = 30) {
    const result = await pool.query('SELECT * FROM identify_regulation_patterns($1, $2)', [
      userId,
      days,
    ]);
    return result.rows;
  ...
  async analyzeWindowOfTolerance(userId, weeks = 12) {
    const result = await pool.query('SELECT * FROM analyze_window_of_tolerance($1, $2)', [
      userId,
      weeks,
    ]);
    return result.rows;
