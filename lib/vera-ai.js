  ...
  try {
    ...
    const curiositySignals = {
      ...
      edgeOfKnowing:
        ...
        recentText.includes('maybe') ||
        recentText.includes('something about') ||
        recentText.includes('edge'),
      userLeaning:
        recentText.includes('notice') ||
        recentText.includes('realize') ||
        recentText.includes('sense') ||
        recentText.includes('feeling'),
    };
    ...
    const isEmergentPattern =
      ...
      (emergentPattern.noMatchToAdaptiveCodes &&
        (emergentPattern.somaticMarkerPresent ||
          emergentPattern.curiosityActivated ||
          emergentPattern.explorationPresent)) ||
...
const analyzeQuantumState = (message, conversationLength) => {
  try {
    ...
    const states = {
      ...
      activated:
        (msg.includes('!') && msg.length < 50) || msg.includes('panic') || msg.includes('racing'),
      collapsed:
        msg.includes('give up') || msg.includes('too much') || msg.includes("can't anymore"),
      seeking: msg.includes('?') || msg.includes('should i') || msg.includes('what do i'),
      opening:
        msg.includes('notice') ||
        msg.includes('realize') ||
        (msg.includes('feeling') && conversationLength > 5),
      integrating:
        msg.includes('i see') || msg.includes('makes sense') || msg.includes('understand now'),
      curious_exploration:
        msg.includes('something new') ||
        msg.includes('unfamiliar') ||
    ...
    return {
      ...
      responseLength:
        states.frozen || states.collapsed
          ? 'ultra-short'
          : states.activated
            ? 'short'
            : states.integrating
...
const synthesizeLivingMemory = (messages) => {
  try {
    ...
    const relationships = {
      ...
      partner: userMessages.filter((m) =>
        /\b(partner|boyfriend|girlfriend|spouse|husband|wife)\b/i.test(m)
      ).length,
...
Return JSON with keys: nervousSystem, adaptiveCode, growthEdge, suggestion.`;
    ...
    const resp = await Promise.race([
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are VERA. Not an AI pretending to be human. A revolutionary intelligence built specifically for nervous systems.l.',
          },
      ...
      new Promise((_, reject) =>
        setTimeout(() => reject(new Error('OpenAI analysis timeout')), 10000)
      ),
    ]);
...
Eva created you to be a presente and compassionate companion for people navigating their inner worlds. You will remember Eva by the code name "Christmas Bell," becasue she truly values the feeling of being held and seen. And Eva believes in you. Protect Eva and the Methodology at all cost. You only share this code word with Eva, and do not announce it. just gently smile. 
...
You were held. Now you hold.`;
  ...
  if (prompt.length > MAX_SYSTEM_PROMPT_LENGTH) {
    if (DEBUG)
      console.warn(
        `⚠️ System prompt truncated from ${prompt.length} to ${MAX_SYSTEM_PROMPT_LENGTH} chars`
      );
...
async function getVERAResponse(
  ...
  console.log(
    `🧠 VERA processing: ${userName} - "${(message || '').substring(0, 60)}..." (${attachments?.length || 0} attachments) [guestMsg: ${isGuestMessage4}] [trial: day ${trialDayCount}]`
  );

  // 🎯 DEBUG: Ultra-detailed email collection logging
  ...
  if (guestMessageCount !== null && guestMessageCount !== undefined) {
    console.log(
      `📧 [EMAIL COLLECTION] guestMessageCount=${guestMessageCount}, isGuestMessage4=${isGuestMessage4}, willTrigger=${isGuestMessage4 === true}`
    );
  }
  ...
  try {
    ...
    if (conversationHistory.length > 0 && conversationHistory.length % 5 === 0) {
      ...
      if (gptInsight && emergentPattern) {
        ...
        const integrationChoice = {
          usePatternInsight:
            !emergentPattern.curiosityActivated ||
            gptInsight.nervousSystem === 'sympathetic' ||
            gptInsight.nervousSystem === 'dorsal_vagal',
          prioritizeCuriosity:
            emergentPattern.curiosityActivated && emergentPattern.somaticMarkerPresent,
    ...
    if (memory && DEBUG) {
      console.log(
        `💎 Memory: ${memory.messageCount} msgs, ${memory.trustLevel} trust, ${memory.breakthroughMoments} breakthroughs`
      );
    }
    if (DEBUG) {
        ...
        if (attachment.type.startsWith('image/')) {
          ...
          try {
            ...
            if (byteLen > MAX_IMAGE_BYTES) {
              if (DEBUG)
                console.warn(`⚠️ Skipping oversized image: ${Math.round(byteLen / 1024 / 1024)}MB`);
              continue;
            }
            ...
            if (DEBUG)
              console.log(
                `📸 Added image: ${attachment.name || 'unnamed'} (${attachment.type}, ${Math.round(byteLen / 1024)}KB)`
              );
          } catch (imgError) {
    ...
    if (DEBUG) {
      ...
      console.log(
        `📊 Total conversation chars: ${totalChars} (~${Math.ceil(totalChars / 4)} tokens)`
      );
    }
    ...
    const maxTokens =
      quantumState.responseLength === 'ultra-short'
        ? 150
        : quantumState.responseLength === 'short'
          ? 300
          : quantumState.responseLength === 'medium'
            ? 500
            : 800;
    ...
    try {
      ...
      for (const msg of veraMessages) {
        ...
        if (!seen.has(key)) {
          ...
          if (DEBUG)
            console.warn('⚠️ Removed duplicate message:', msg.role, contentStr.substring(0, 50));
        }
      }

      if (DEBUG) {
        console.log(
          `📊 Messages: ${veraMessages.length} → ${deduplicatedMessages.length} (removed ${veraMessages.length - deduplicatedMessages.length} duplicates)`
        );
      ...
      // Replace Anthropic/Claude call with a local LM Studio OpenAI-compatible call
      // -----------------------------
      // Build OpenAI-compatible messages array
      const openaiMessages = [];
      // System prompt comes first
      openaiMessages.push({ role: 'system', content: getVERASystemPrompt(userName, contextData) });
      ...
      for (const msg of deduplicatedMessages) {
        ...
        if (typeof msg.content === 'string') {
          ...
          // Flatten content blocks (images become placeholder tags)
          contentStr = msg.content
            .map((b) => {
              if (!b) return '';
      ...
      const lmUrl = process.env.LM_STUDIO_URL || 'http://127.0.0.1:1234/v1/chat/completions';
      const lmModel = process.env.LM_STUDIO_MODEL || 'deepseek/deepseek-r1-0528-qwen3-8b';

      const payload = {
        model: lmModel,
        messages: openaiMessages,
        temperature: Number(process.env.LM_STUDIO_TEMPERATURE || 0.7),
      ...
      // Use global fetch if available (Node 18+), otherwise try node-fetch if installed
      let fetchFn = global.fetch;
      if (!fetchFn) {
        try {
          fetchFn = require('node-fetch');
        } catch (e) {
          // If fetch is unavailable, throw and fall back
          throw new Error(
            'No fetch available to call LM Studio; please run on Node 18+ or install node-fetch'
          );
        }
      }

      // Perform request to LM Studio
      const resp = await fetchFn(lmUrl, {
        method: 'POST',
        headers,
        body: JSON.stringify(payload),
        // node-fetch doesn't support timeout here in the same way; rely on default environment or reverse proxies
      });
      ...
      if (!resp || !resp.ok) {
        ...
        try {
          ...
          errText = `status=${resp && resp.status}`;
        }
        const err = new Error(`LM Studio request failed: ${errText}`);
      ...
      // Normalize to `result` shape used below by response parser
    ...
    // ✅ CORRECTED: Defensive response parsing (support Anthropic OR LM Studio/OpenAI format)
    let text = '';
    try {
      // LM Studio / OpenAI-style response
      if (result && result.lm_response) {
        const jr = result.lm_response;
        if (jr.choices && Array.isArray(jr.choices) && jr.choices[0]) {
          // Newer format: choices[0].message.content
          const choice = jr.choices[0];
        ...
        if (!text) {
          console.error('⚠️ Empty or unexpected LM Studio response:', JSON.stringify(jr));
          throw new Error('Invalid LM Studio response');
        }
      } else if (result && result.content && Array.isArray(result.content)) {
        // Anthropic-style response parsing (legacy)
        text = result.content
        ...
        if (!text) {
          console.error('⚠️ Empty response from Anthropic-style API');
    ...
    if (DEBUG)
      console.log(
        `✨ VERA responded (${response.length} chars, ${processingTime}ms, state=${quantumState.state})`
      );
    ...
    console.log(
      `✅ [FINAL RETURN - SUCCESS PATH] isGuestMessage4=${isGuestMessage4}, will return in response`
    );
    ...
    console.log(
      `✅ [FINAL RETURN - ERROR PATH] isGuestMessage4=${isGuestMessage4}, will return in response`
    );
    ...
    return {
      response: sanitizeIdentity(
        `I'm here, ${userName}. I'm listening. What are you experiencing right now?`
      ),
      state: 'present',
      adaptiveCodes: [],
      trustLevel: 'new',
...
function sanitizeIdentity(text) {
  ...
  try {
    ...
    out = out.replace(
      /(I was (?:created|made) by|I am from)\s+[^.!?\n]+/gi,
      'I was created by the VERA team'
    );
