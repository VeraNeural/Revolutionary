...
async function getVERAResponse(userId, message, userName, pool, attachments = [], guestMessageCount = null) {
  const startTime = Date.now();
  
  // Check if this is the 4th guest message to trigger email collection
  const isGuestMessage4 = guestMessageCount === 4;
  
  console.log(`🧠 VERA processing: ${userName} - "${(message || '').substring(0, 60)}..." (${attachments?.length || 0} attachments) [guestMsg: ${isGuestMessage4}]`);
  ...
  try {
    ...
    // Add email collection prompt if this is the 4th guest message
