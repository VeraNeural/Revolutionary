...
async function testStripe() {
  ...
  console.log('Key ends with:', process.env.STRIPE_SECRET_KEY?.substring(process.env.STRIPE_SECRET_KEY.length - 5));
  ...
  try {
    ...
    if (error.code === 'ERR_INVALID_CHAR') {
      ...
      console.error('⚠️ Copy the key again from Stripe dashboard - be careful not to add extra whitespace');
    }
  }
}
